...
def main():
    ...
    for i in range(num_runs):
        print(i, '/', num_runs)
        t0 = time.time()
        l1_model = models.get_trained_l1_all_digit_model(train_images, train_labels, epochs=1)
        l1_model.save('models/l1_cifar10_model' + str(i))

        t1 = time.time()
        l2_model = models.get_trained_l2_all_digit_model(train_images, train_labels, input_shape, epochs=1)
        l2_model.save('models/l2_cifar10_model' + str(i))

        t2 = time.time()
        l3_model = models.get_trained_l3_all_digit_model(train_images, train_labels, input_shape, epochs=1)
        l3_model.save('models/l3_cifar10_model' + str(i))

        t3 = time.time()
        # l4_model = models.get_trained_l4_all_digit_model(train_images, train_labels, input_shape, epochs=1)
