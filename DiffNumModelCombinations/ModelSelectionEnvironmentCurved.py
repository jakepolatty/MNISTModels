...
class ModelSelectionEnvironment(Environment):
    ...
    def curved_reward(self, terminal):
        if terminal:
            model_cost = self.get_called_model_cost()
            #print("COST", model_cost)

            if self.is_correct_prediction():
                frac = (model_cost - self.min_cost) / (self.max_cost - self.min_cost)
                return 1 - (frac ** self.alpha)
            else:
                frac = (model_cost - self.min_cost) / (self.max_cost - self.min_cost)
                return -0.1 - (frac ** self.alpha)
        else:
            return 0.0
    ...
    def execute(self, actions):
        ...
        reward = self.curved_reward(terminal)
        #print("REWARD", reward)
