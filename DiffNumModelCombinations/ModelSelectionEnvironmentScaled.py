...
class ModelSelectionEnvironment(Environment):
    ...
    def __init__(self, num_models, output_size, val_model_outputs, y_val,
            test_model_outputs, y_test, avg_model_costs, weight_table):
        super().__init__()
    ...
    def compute_timestep(self, action):
        ...
        if action < self.num_models:
            ...
            weights = self.weight_table[action]

            self.state = np.multiply(model_output, weights)
